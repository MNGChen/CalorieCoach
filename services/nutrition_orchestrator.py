...
class NutritionOrchestrator:
    ...
    def _router(self,s):
        try: result=self.router.route(s["message"]); return {"intent":result.intent,"trace":s["trace"]+["router"]}
        except RouterError: return {"intent":"general_nutrition_chat","trace":s["trace"]+["router_error"]}
    def _log(self,s):
        try:
            foods=self.food.analyze(s["message"]); saved=self.logging.save_resolved(s["message"],foods)
    ...
    def _recommend(self,s):
        ...
        try: plan=self.planner.recommend(advice); return {"response":{"intent":"meal_recommendation","message":f"Suggested: {plan['meal_name']}","data":{"coach":advice,"meal_plan":plan},"trace":s["trace"]+["daily_tracking","coach_agent","meal_planning_agent","portion_calculator"]}}
