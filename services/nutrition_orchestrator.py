...
class NutritionOrchestrator:
    ...
    def handle(self,message:str)->dict[str,Any]:
        ...
        state=self.graph.invoke({"message":message,"trace":[],"errors":[],"memory_context":memory_context})
    ...
    def _coach(self,s):
        ...
        advice=self.coach.get_nutrition_advice(s["message"], memory_context=s.get("memory_context")); return {"response":{"intent":"nutrition_question","message":advice.get("summary",advice.get("reason","Your daily summary is available.")),"data":advice,"trace":s["trace"]+["daily_tracking","coach_agent"]}}
    def _recommend(self,s):
        self._notify("Getting guidance from your current nutrition data…")
        advice=self.coach.get_nutrition_advice(s["message"], memory_context=s.get("memory_context"))
