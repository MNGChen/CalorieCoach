...
class MealPlanningService:
    ...
    def recommend(self, coach_result: dict[str, Any], recent_foods: list[str] | None = None) -> dict[str, Any]:
        ...
        candidates = self.retrieve_candidates(coach_result.get("priority", ""), recent_foods or [])
        if not candidates: raise MealPlanningAgentError("No suitable local food candidates are available.")
        context = {"user_goal": coach_result.get("user_goal"), "remaining_today": coach_result.get("daily_summary", {}).get("remaining", {}), "target_for_next_meal": target, "strategy": coach_result.get("priority", ""), "recent_foods": recent_foods or [], "candidates": candidates}
        ...
        for attempt in range(2):
            ...
            last = calculated
        return last or {}
    def retrieve_candidates(self, strategy: str, recent_foods: list[str]) -> list[dict[str, Any]]:
