...
def render_metric_cards(
    ...
    for col, label, consumed, target, unit in (
        (columns[0], "Calories", calories, calorie_goal, "kcal"),
        (columns[1], "Protein", protein, protein_goal, "g"),
        (columns[2], "Carbohydrates", carbs, carbs_goal, "g"),
        (columns[3], "Fat", fat, fat_goal, "g"),
    ):
        remaining = target - consumed
        col.metric(label, f"{consumed:.0f} / {target:.0f} {unit}", delta=f"{remaining:.0f} {unit} remaining")
...
def _render_goal_bar(label: str, current: float, target: float, icon: str, unit: str = "kcal") -> None:
    ...
    st.markdown(
        ...
        <div style="display:flex;justify-content:space-between;font-size:0.9rem">
          <span>{icon} {label}{status}</span><span>{current:.0f} / {target:.0f} {unit} · {remaining:.0f} remaining</span>
        </div>
